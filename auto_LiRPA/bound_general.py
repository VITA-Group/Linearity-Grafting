...
class BoundedModule(nn.Module):
    ...
    def __init__(self, model, global_input, bound_opts=None, auto_batch_dim=True, device='auto',
        ...
        if bound_opts is None:
            bound_opts = {}
        # Default options.
        default_bound_opts = {'ibp_relative': False, 'conv_mode': 'patches', 'sparse_intermediate_bounds': True,
                'sparse_conv_intermediate_bounds': True, 'sparse_intermediate_bounds_with_ibp': True}
    ...
    def forward(self, *x, final_node_name=None, clear_forward_only=False):
        ...
        while len(queue) > 0:
            l = queue.popleft()
            inp = [l_pre.forward_value for l_pre in l.inputs]
    ...
    def get_unstable_conv_locations(self, node, ref_intermediate_lb, ref_intermediate_ub):
        ...
        # Get mask 
        # graft_mask = None
        # mask_name_list = self._modules[node.output_name[0]].input_name
        # for mask_name in mask_name_list:
        #     if '/mask' in mask_name:
        #         maped_mask_name = self.node_name_map['{}.buffer'.format(mask_name)]
        #         if not maped_mask_name in self.state_dict().keys():
        #             maped_mask_name = '/mask.{}'.format(maped_mask_name)
        #         graft_mask = self.state_dict()[maped_mask_name]

        # if graft_mask is not None:
        #     unstable_masks = unstable_masks * (1 - graft_mask)
    ...
    def get_unstable_locations(self, node, ref_intermediate_lb, ref_intermediate_ub):
                    ...
                    if not hasattr(node, 'lower'):
                        ...
                        if forward:
                            ...
                            if (isinstance(node, BoundActivation) or isinstance(node, BoundTranspose)) and hasattr(
                                    node.inputs[0], 'lower'):
                                node.lower = node.forward(node.inputs[0].lower)
                                node.upper = node.forward(node.inputs[0].upper)
                                ...
                                if not first_layer_flag:
                                    ...
                                    if sparse_intermediate_bounds:
                                        if node.name not in aux_reference_bounds:
                                            ...
                                            if sparse_intermediate_bounds_with_ibp:
                                                with torch.no_grad():
                                                    ...
                                                    ref_intermediate_lb, ref_intermediate_ub = self._IBP_general(node=node, delete_bounds_after_use=True)
                                            else:
                                                sparse_intermediate_bounds = False
                                        else:
                                            ref_intermediate_lb, ref_intermediate_ub = aux_reference_bounds[node.name]
                                    ...
                                    if (isinstance(node, BoundLinear) or isinstance(node, BoundMatMul)) and int(
                                        ...
                                        if sparse_intermediate_bounds:
                                            ...
                                            unstable_idx, unstable_size = self.get_unstable_locations(node, ref_intermediate_lb, ref_intermediate_ub)
                                            if unstable_size == 0:
                                                ...
                                                # print(f'layer {node} {node.name} skipped, unstable_size {unstable_size} dim {dim} thresh {0.9 * dim}')
                                                unstable_idx = None
                                        ...
                                        if sparse_intermediate_bounds:
                                            unstable_idx, unstable_size = self.get_unstable_conv_locations(node, ref_intermediate_lb, ref_intermediate_ub)
                                            if unstable_size == 0:
                                                # Do nothing, no bounds will be computed.
                                                reduced_dim = True
                                                unstable_idx = []
                                            # We sum over the channel direction, so need to multiply that.
                                                ...
                                                # print(f'layer {node} {node.name} skipped, unstable_size {unstable_size} dim {dim} thresh {0.8 * dim}')
                                                unstable_idx = None
                                                del ref_intermediate_lb, ref_intermediate_ub
                                    ...
                                    # print(f'node {node.name} unstable_size {unstable_size}')
                                    ...
                                    if reduced_dim:
                                        if unstable_size > 0:
                                            ...
                                            if isinstance(unstable_idx, tuple):
                                                new_lower = ref_intermediate_lb.detach().clone()
                                                new_upper = ref_intermediate_ub.detach().clone()
                                                ...
                                                # Other layers.
                                                new_lower = ref_intermediate_lb.detach().clone().view(batch_size, -1)
                                                new_upper = ref_intermediate_ub.detach().clone().view(batch_size, -1)
                                                new_lower[:, unstable_idx] = node.lower.view(batch_size, -1)
                                                new_upper[:, unstable_idx] = node.upper.view(batch_size, -1)
                                            # print(f'{node.name} {node} bound diff {(new_lower.view(-1) - ref_intermediate_lb.view(-1)).sum()} {(new_upper.view(-1) - ref_intermediate_ub.view(-1)).sum()}')
    ...
    def _IBP_general(self, node=None, C=None, delete_bounds_after_use=False):
        ...
        if not hasattr(node, 'interval'):
            for n in node.inputs:
                if not hasattr(n, 'interval'):
                    ...
                    self._IBP_general(n, delete_bounds_after_use=delete_bounds_after_use)
    ...
    def _backward_general(self, C=None, node=None, root=None, bound_lower=True, bound_upper=True,
        ...
        while len(queue) > 0:
            ...
            for l_pre in l.inputs:
                all_nodes_before.append(l_pre.name)
                degree_out[l_pre.name] += 1  # calculate the out degree
                if l_pre.bounded:
                    l_pre.bounded = False
                    queue.append(l_pre)
        node.bounded = True
        ...
        while len(queue) > 0:
            ...
            for l_pre in l.inputs:  # if all the succeeds are done, then we can turn to this node in the next iteration.
            ...
            if l.lA is not None or l.uA is not None:
                ...
                if isinstance(l, BoundRelu):
                    ...
                    A, lower_b, upper_b = l.bound_backward(l.lA, l.uA, *l.inputs)
                # print(f'start node {node.name} after layer {l.name} A norm {self._get_A_norm(A[0])}')
                ...
                for i, l_pre in enumerate(l.inputs):
                    add_bound(l_pre, lA=A[i][0], uA=A[i][1])
        ...
        if not hasattr(node, 'linear'):
            for l_pre in node.inputs:
                if not hasattr(l_pre, 'linear'):
                    self._forward_general(node=l_pre, root=root, dim_in=dim_in)
