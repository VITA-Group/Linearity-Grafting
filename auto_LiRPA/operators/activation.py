...
class BoundRelu(BoundOptimizableActivation):
    ...
    def _backward_relaxation(self, last_lA, last_uA, x, start_node, unstable_idx):
        ...
        if hasattr(x, 'interval') and Interval.use_relative_bounds(x.interval):
            # Relative interval bound (center, diff).
            ...
            # Absolute interval bound (lb, ub).
            ub_r = torch.max(ub_r, lb_r + 1e-8)
            # Upper bound slope, not optimized.
            upper_d = ub_r / (ub_r - lb_r)
        # Upper bound bias, not optimized.
    ...
    def bound_backward(self, last_lA, last_uA, x=None, start_node=None, beta_for_intermediate_layers=False, unstable_idx=None):
        ...
        upper_d, upper_b, lower_d, lower_b, lb_lower_d, ub_lower_d = self._backward_relaxation(last_lA, last_uA, x, start_node, unstable_idx)
        ...
        def _bound_oneside(last_A, d_pos, d_neg, b_pos, b_neg):
            ...
            if type(last_A) == torch.Tensor:
                ...
                if last_A.identity == 0:
                    ...
                    prod, bias = multiply_by_A_signs(patches, d_pos, d_neg, b_pos, b_neg, patches_mode=True)
        ...
        lower_b = _maybe_unfold(lower_b, last_lA if last_lA is not None else last_uA)  # for ReLU it is always None; keeping it here for completeness.
        ub_lower_d = _maybe_unfold(ub_lower_d, last_uA)
        lb_lower_d = _maybe_unfold(lb_lower_d, last_lA)

        uA, ubias = _bound_oneside(last_uA, upper_d, ub_lower_d if lower_d is None else lower_d, upper_b, lower_b)
        lA, lbias = _bound_oneside(last_lA, lb_lower_d if lower_d is None else lower_d, upper_d, lower_b, upper_b)
