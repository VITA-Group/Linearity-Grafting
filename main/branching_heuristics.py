...
@torch.no_grad()
def choose_node_parallel_kFSB_grafting(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs,
    ...
    grafting_mask = []
    num_idx = 0
    for key in net.net.state_dict().keys():
        if key[-5:] == '.mask':
            mask_weight = net.net.state_dict()[key].reshape(1, -1)
            grafting_mask.append(mask_weight)
            num_idx += 1
    print('* Mask layer = {}'.format(len(grafting_mask)))
    ...
    # Compute BaBSR scores, starting from the last layer. (layer-wise calculate score)
    for layer_i, layer in enumerate(reversed(net.net.relus)):
        this_layer_mask = mask[relu_idx]
        this_layer_mask_grafting = grafting_mask[relu_idx]
        ...
        # set the neuron in grafting mask to inf for adjusted_intercept_candidate
        adjusted_intercept_candidate += OFFSET * this_layer_mask_grafting
        intercept_tb.insert(0, adjusted_intercept_candidate) ## List, scores 
        ...
        # Slope score, the main score in BaBSR. A higher (more positive) score is better. (-inf)

        # set the neuron in grafting mask to -inf for adjusted_intercept_candidate
        adjusted_score_candidate -= OFFSET * this_layer_mask_grafting
    ...
    all_score = torch.cat(score, dim=1)  ### split 
    all_itb = torch.cat(intercept_tb, dim=1) ### split
    ...
    score_idx_indices = score_idx.indices.cpu()
    itb_idx = torch.topk(all_itb, topk, largest=False)  # k-smallest elements. #### unstable neuron, 
    itb_idx_indices = itb_idx.indices.cpu()
